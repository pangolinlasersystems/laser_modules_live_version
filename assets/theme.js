...
$(document).ready(function() {
  if ($(window).width()<=950) {
    $(".link_list-wrap").addClass("footer-accorion");
  } else {
    $(".link_list-wrap").removeClass("footer-accorion");
  }
});
...
$(document).ready(function() {
  $(".link_list-wrap").click(function(e) {
    $(this).find(".site-footer__linklist").toggleClass("active-link-list");
    $(this).find(".h4").toggleClass("hide-icon");
  });
});
...
$(function() {
  $(".mobile-drawer-icon").click(function() {
	$(".mobile-drawer-menu").addClass("open-drawer-menu");
    $(".mobile-nav-overlay").addClass("active-overlay");
  });
  
  $(".mobile-nav-overlay").click(function() {
    $(".mobile-drawer-menu").removeClass("open-drawer-menu");
...
$('.dropdown-icon').click(function() {
  ...
  $(this).prev().toggleClass("active-link");
  $(this).find('.icon-minus').toggleClass("active-icon");
  $(this).find('.icon-chevron-down').toggleClass("hide-icon");
});
...
$(function() {
  $(".collection-grid-style-btn").click(function() {
    if ( $(this).hasClass( "active-btn" ) ) {
    
    } else {
      $(this).addClass("active-btn");
      $(".collection-list-style-btn").removeClass("active-btn");
      $(".collection-grid-wrap").addClass("active-collection-style");
      $(".list-view-items").removeClass("active-collection-style");

    }   
  });

  $(".collection-list-style-btn").click(function() {
    if ( $(this).hasClass( "active-btn" ) ) {

    } else {
      $(this).addClass("active-btn");
      $(".collection-grid-style-btn").removeClass("active-btn");
      $(".list-view-items").addClass("active-collection-style");
      $(".collection-grid-wrap").removeClass("active-collection-style");
    }   
  }); 
});

// Product Show More/Less btns

$('.prod_description__item').on('click', '.text-link.expand-open',function(e){
  e.preventDefault();
  $(this).parent().toggleClass('active-item');
});


$('.prod_description__item').on('click', '#specification-btn-text',function(e){
  e.preventDefault();
  var varText = $(this).text();

  if(varText.match('Show Full Specification')) {
    $(this).html('Show Less Specification');
  } else {
    $(this).html('Show Full Specification');
  }  
});

$('.prod_description__item').on('click', '#description-btn-text',function(e){
  e.preventDefault();
  var varText = $(this).text();

  if(varText.match('Show Full Description')) {
    $(this).html('Show Less Description');
  } else {
    $(this).html('Show Full Description');
  }  
});

// Product Options Price Calculating

$(document).on('click', '.swatch-option', function(e) {
  setTimeout(function(){
    var varTotalMoney = $(document).find('#price--main').text();    
    var varProductMoney = $(document).find('.price-item--regular').text();
    
    console.log(varTotalMoney);
    console.log(varProductMoney);

    var varTotalTotal = varTotalMoney.replace(/[,]/g,"").split("€");
    var varTotal = Number (varTotalTotal[0]);
    
        console.log(varTotal);

    var varPricePrice = varProductMoney.replace(/[,]/g,"").replace( /\s/g, '').split("€");
    var varPrice = Number (varPricePrice[0]);
    
        console.log(varPrice);

    var differencePrice = (varTotal - varPrice);

    if (differencePrice > 999) {
      var bigDifferencePrice = (differencePrice / 1000).toFixed(3);

      var stringPrice = '' + bigDifferencePrice;
      var toStringPrice = stringPrice.replace(/[.]/g,",");

      $(document).find('#differencePrice').html('<p class="prod-subtotal-price">' + (toStringPrice) + '€' + '</p>');
    } else {
      $(document).find('#differencePrice').html('<p class="prod-subtotal-price">' + (differencePrice) + '€' + '</p>');
    }
  }, 400);	
});	
...
$(document).ready(function () {
  $('.template-product').on('click','.quote-btn', function(e) {
    e.preventDefault();

    $('.contact_for_quote').fadeIn(350);  
  });

  $('.contact_for_quote').on('click','.contact_for_quote_close', function(e) {
    e.preventDefault();
    $('.contact_for_quote').fadeOut(350);
  });

  $(document).mouseup(function (e) {
    var container = $(".contact_for_quote");
...
$(function () {
  $(".testimonial-box").slice(0, 6).show();
  $("body").on('click touchstart', '#testimonals-btn', function (e) {
    e.preventDefault();
    $(".testimonial-box:hidden").slice(0, 6).slideDown();
    if ($(".testimonial-box:hidden").length == 0) {
      $("#testimonals-btn").css('visibility', 'hidden');
    }
    $('html,body').animate({
...
$(this).find('.cd-timeline-img, .cd-timeline-content').removeClass('is-hidden').addClass('bounce-in');
 ...
 $(".download_pdf_button").on("click", function(e){
    ...
    var product_obj = JSON.parse($("[id^='ProductJson-']").html());
    var image_src = product_obj.images[0];
    toDataURL(image_src, function(dataUrl) {
    var description = $(".description-tab").html();
    var specifications = $(".specifications-tab .scrollable-wrapper").html();
    var tag = $(".product-template__container").data("pdf-tag");
      ...
      var json = JSON.stringify({
        firstName: $(".product-bg h1").html(),
        image: dataUrl,
        description: description,
        specifications: specifications,
        footer: "www.kvantlasers.sk | info@kvant.sk | Odborárska 23,831 02 Bratislava,Slovakia"
      });

      var xhr = new XMLHttpRequest();
      xhr.open('POST', 'https://render-and-download-pdf.herokuapp.com/generate-pdf', true);
      ...
      xhr.onload = function(e) {
        if (this.status == 200) {
          ...
          link.download= $(".product-bg h1").html() +".pdf";
          link.click();
...
// $('.mw_options_list').find('li input').click(function() {
//   var text = $('.mw_options_list').find('li span').text();

//   $('#btnOptions').append(text);
// });